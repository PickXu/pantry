...
public class CCompiler {
  ...
  /**
   * If true, implicit truncating casts are added during arithmetic
   * and assignment of numeric types, as per the ANSI C standard. 
   * 
   * If false, no truncating casts are added unless they are explicit 
   * (i.e. (uint32_t)0xF00000000L will truncate)
   */
  ...
  private static class CLexicalScope {
    ...
    /**
     * If lexicalReset is true, variable lookups that try to read from a parent scope will be disallowed, 
     * and will jump to lookup in the global variables.
     * 
     * I.e. lexicalReset is true when inlining a function call, to ensure static scoping.
     */
    ...
    public LvalExpression addVar(String string, Type cType, boolean isOutput) {
      ...
      if (alreadyExists==null){
        alreadyExists = Function.getVars().addVar(getAddID(string), cType, false, isOutput);
      }
      return alreadyExists;
    }
  }
  private void addGlobalVariable(CLexicalScope scope, VariableDeclaration q, StatementBuffer sb) {
    ...
    if (isInput){
      ...
      if (q.initializer != null){
        ...
        //Global variables with the type specifier "const" are added to the consts list, if possible
        if (q.type.spec.contains("const")){
          //gvar is out of date
          gvar = gvar.changeReference(Function.getVars());
          ConstExpression ce = ConstExpression.toConstExpression(gvar);
          if (ce != null){
  ...
  private void addInitializerStatement(LvalExpression gvar, CLexicalScope scope, Expression initializer, StatementBuffer sb) {
    if (initializer instanceof MultiExpression){
      ...
      for(int i = 0; i < me.size(); i++){
        /*
        addStatement(new ProtoAssignmentStatement(
            new SFE.Compiler.BinaryOpExpression(new ArrayAccessOperator(), gvar, IntConstant.valueOf(i)), 
            new SFE.Compiler.UnaryOpExpression(new UnaryPlusOperator(), getExpr(scope, me.get(i), sb)),
            null
            ), sb);
            */
        //Recursive method to allow multidimensional array initializers
        if (gvar.getDeclaredType() instanceof ArrayType){
          LvalExpression component = 
              new ArrayAccessOperator().resolve(gvar, IntConstant.valueOf(i));
          ...
          LvalExpression component = 
              new StructAccessOperator(fieldname).resolve(gvar);
          addInitializerStatement(component, scope, me.get(i), sb);
        } else {
          throw new RuntimeException("Cannot assign multiexpression "+ me+" to lvalue "+gvar);
        }
      }
    } else {
    ...
    public void toAssignmentStatements(StatementBuffer sb) {
      ...
      for(Object q : program){
        if (q instanceof FunctionDefinition){
          ...
          if (q2.type instanceof TypeDefSpecification){ //Strangely, typedefs parse through as variable declarations.
      ...
      SFE.Compiler.Expression returnValue_ = expandStatement(main_s, main_d.block, sb);
   ...
   */
  private static void addStatement(SFE.Compiler.Statement s, StatementBuffer sb) {
   ...
   * @param sb 
   */
  private SFE.Compiler.Expression expandStatement(CLexicalScope scope, Statement statement, StatementBuffer sb) {
    if (statement instanceof CompoundStatement) {
      ...
      for(Object s_or_d : s){
        if (s_or_d instanceof Declaration){
          ...
          if (d instanceof MultiVariableDeclaration){
            ...
            for(VariableDeclaration v : d2){
              ...
              if (v.initializer != null){
                addInitializerStatement(var, scope, v.initializer, sb);
              }
            }
          } else {
          ...
          throw new RuntimeException("Assertion error: statement_declaration_list held an invalid object: "+s_or_d.getClass());
        }
      }
      scope.popScope();
      ...
      getExpr(scope, (Expression)statement, sb); //But disregard the value returned.
      return null;
      ...
      while(true){
        ...
        if (condValue == null){
          ...
          //Create a loop statement whose pre is the body, with the same body and condition.
          LoopStatement nextIteration = new LoopStatement(loop.block, loop.enter, loop.block, new CompoundStatement());
          IfStatement possibleNextIteration = new IfStatement(cond, nextIteration, new CompoundStatement());
      ...
      */
      ...
      scope.popScope();
      //If we have an else block, output it.
      SFE.Compiler.Expression retValEls;
      BlockStatement els;
      if (fib.els != null){
        els = new BlockStatement();
        scope.pushScope("else");
        retValEls = expandStatement(scope, fib.els, els);
        scope.popScope();
      } else {
        retValEls = null;
        els = null;
      ...
      */
      ...
      throw new RuntimeException("I don't know how to coerce to statement: "+statement);
    }
  }
  //private static int if_condition_uid = 0;
  //private static int return_value_uid = 0;
  private SFE.Compiler.Expression getExpr(CLexicalScope scope, Expression ein, StatementBuffer sb) {
    if (ein instanceof BinaryOpExpression) {
      ...
      SFE.Compiler.Expression toRet = new SFE.Compiler.UnaryOpExpression(new CStyleCast(castType), getExpr(scope, cein.toCast, sb));
    ...
    } else if (ein instanceof IfStatement || ein instanceof ConditionalExpression){
      ...
      if (ein instanceof IfStatement){
        ...
        then_ = cein.the;
        else_ = cein.els;
        muxLval = scope.addVar(":conditional_exp_"+(parse_uid), new BusType(), false);
        addStatement(asId(muxLval, IntConstant.ZERO), sb);
      }
      
      //Evaluate condition to an lvalue
      LvalExpression cond_boolean = scope.addVar(":condition"+(parse_uid), new BooleanType(), false);
      addStatement(asId(cond_boolean, 
          new SFE.Compiler.BinaryOpExpression(new NotEqualOperator(), cond, new BooleanConstant(false))), sb);
      cond_boolean = cond_boolean.changeReference(Function.getVars());
      
      BooleanConstant asConst = BooleanConstant.toBooleanConstant(cond_boolean);
      ...
      SFE.Compiler.IfStatement.expandMuxStatements(cond_boolean, thenScope, elseScope, sb);
      ...
      if (ein instanceof IfStatement){
        ...
        muxLval = muxLval.changeReference(Function.getVars()); //This is safe, muxLval is an lvalexpression.
        ...
        if (asConst != null){
          ...
          muxLval.metaType = CTypeForBOE(thenret_metatype, new MinusOperator(), elseret_metatype);
        }
        return muxLval;
      }
      /*
      ...
      LvalExpression muxLval = scope.addVar(":conditional_exp_"+(if_condition_uid++), new BusType(), false);
      ...
      */
      ...
      return genericFunctionCall(scope, fc.funcName.identifier, fc.parse_uid, sb, inputExprs);
      ...
      SFE.Compiler.Expression lhsLookup = getExpr(scope, ast.LHS_lookup, sb);
  ...
  private SFE.Compiler.Expression genericFunctionCall(CLexicalScope scope,
    ...
    SFE.Compiler.Expression asBuiltIn = builtinFunctionCall(scope, funcName, parse_uid, sb, inputExprs);
    if (asBuiltIn != null){
      return asBuiltIn;
    }
    ...
    for(int i = 0; i < toCallInfo.args.size(); i++){
      ...
      if (typeOfArgument instanceof ArrayType){
        ...
        if (typeOfArgument_.getComponentType() instanceof ArrayType){
          throw new RuntimeException("Multidimensional arrays as arguments to function "+funcName+" not yet supported");
        } else {
          //Replace typeOfArgument with pointer-to-basetype
          typeOfArgument = new PointerType(typeOfArgument_.getComponentType());
        }
      }
      LvalExpression addVar = scope.addVar(vd.name.identifier, typeOfArgument, false);
      addStatement(asId(addVar, inputExprs.get(i)), sb);
    }
    
    SFE.Compiler.Expression returnValue_ = expandStatement(scope, toCall.block, sb);
    scope.popScope();
    if (returnValue_ == null){
      return null;
    }
    ...
    SFE.Compiler.Expression returnValue = returnValue_.evaluateExpression(":"+funcName+(parse_uid), "returnvalue", result);
  ...
  private CStyleCast cCastForAssignment(SFE.Compiler.Expression lhsLookup) {
    ...
    if (lhsLookup.metaType instanceof ArrayType){
      ...
      cc = new CStyleCast(lhsLookup.metaType); //Handle assigning arrays to pointers.
    } else {
  ...
  private SFE.Compiler.Expression builtinFunctionCall(CLexicalScope scope, String funcName, int parse_uid, StatementBuffer sb, ArrayList<SFE.Compiler.Expression> args) {
    if (funcName.equals(CBuiltinFunctions.RAMGET_NAME)){
      ...
      if (!(pointerToTargetArg.metaType instanceof PointerType)){
        throw new RuntimeException("Pointer to data to get in ramget, "+pointerToTargetArg+" was not a pointer.");
      }
      LvalExpression ptrToTarget = scope.addVar("ptrToTarget", pointerToTargetArg.metaType, false);
      addStatement(asId(ptrToTarget, pointerToTargetArg), sb);

      ArrayList<LvalExpression> addrs = new ArrayList();
      for(int i = 1; i < args.size(); i++){
        LvalExpression addVar = scope.addVar("addr"+(i-1), new IntType(), false);
    ...
    if (funcName.equals(CBuiltinFunctions.RAMPUT_NAME)){
      ...
      if (!(pointerToValArg.metaType instanceof PointerType)){
        throw new RuntimeException("Pointer to data to put in ramput, "+pointerToValArg+" was not a pointer.");
      }
      LvalExpression pointerToValue = scope.addVar("ptrToData", pointerToValArg.metaType, false);
    ...
    if (funcName.equals(CBuiltinFunctions.HASHPUT_NAME)){
      ...
      //Initialize the inputs to the function
      LvalExpression pointerToReceiveHash = scope.addVar("ptrToRecvHash", new PointerType(typetable.get("hash_t")), false);
      if (!(args.get(1).metaType instanceof PointerType)){
        throw new RuntimeException("Second argument to hashput, "+args.get(1)+" was not a pointer.");
      }
      LvalExpression pointerToValue = scope.addVar("ptrToData", new PointerType(new AnyType()), false);
      addStatement(asId(pointerToReceiveHash, args.get(0)), sb);
      addStatement(asId(pointerToValue, args.get(1)), sb);
      
      addStatement(new HashPutStatement(pointerToReceiveHash, pointerToValue), sb); 
      scope.popScope();
      return VoidRetVal;
    }
    ...
    if (funcName.equals(CBuiltinFunctions.HASHGET_NAME)){
      ...
      if (!(pointerToRecvDataArg.metaType instanceof PointerType)){
        throw new RuntimeException("First argument to hashget, "+pointerToRecvDataArg+" was not a pointer.");
      }
      LvalExpression pointerToRecvData = scope.addVar("ptrToRecvDat", new PointerType(new AnyType()), false);
      LvalExpression pointerToHash = scope.addVar("ptrToHash", new PointerType(typetable.get("hash_t")), false);
      addStatement(asId(pointerToRecvData, pointerToRecvDataArg), sb);
      addStatement(asId(pointerToHash, args.get(1)), sb);
      
      addStatement(new HashGetStatement(pointerToRecvData, pointerToHash), sb); 
      scope.popScope();
      return VoidRetVal;
    }
    ...
    if (funcName.equals(CBuiltinFunctions.HASHFREE_NAME)){
      ...
      if (!(pointerToHashArg.metaType instanceof PointerType)){
        throw new RuntimeException("First argument to hashfree, "+pointerToHashArg+" was not a pointer.");
      }
      LvalExpression pointerToHash = scope.addVar("ptrToHash", new PointerType(typetable.get("hash_t")), false);
    ...
    if (funcName.equals(CBuiltinFunctions.INT_STRING_TO_BITS)){
      //Initialize the inputs to the function
      Pointer output = Pointer.toPointer(args.get(0));
      Pointer string = Pointer.toPointer(args.get(1));
      ...
      int index = 0;
      while(true){
        char char_val = (char) IntConstant.toIntConstant(string.increment(index++).access()).toInt();
        if (char_val == 0){
          break;
        }
        buf.append(char_val);
      }
      IntConstant parsed = IntConstant.valueOf(new BigInteger(buf.toString(), radix));
      
      BitStringHelper.BitStringStoreTarget bsst = BitStringHelper.toBitStringStoreTarget(output);
      ...
      BitString parsed_bs = BitString.toBitString(new RestrictedSignedIntType(bitsToRecvDat.size()), sb, parsed);
    ...
    if (funcName.equals(CBuiltinFunctions.COMMITMENTGET_NAME)){
      ...
      SFE.Compiler.Expression pointerToRecvDataArg = args.get(0);
      if (!(pointerToRecvDataArg.metaType instanceof PointerType)){
        throw new RuntimeException("First argument to "+CBuiltinFunctions.COMMITMENTGET_NAME+", "+pointerToRecvDataArg+" was not a pointer.");
      }
      LvalExpression pointerToRecvData = scope.addVar("ptrToRecvDat", new PointerType(new AnyType()), false);
      LvalExpression pointerToHash = scope.addVar("ptrToHash", new PointerType(typetable.get("commitment_t")), false);
      addStatement(asId(pointerToRecvData, pointerToRecvDataArg), sb);
      addStatement(asId(pointerToHash, args.get(1)), sb);
      pointerToRecvData = pointerToRecvData.changeReference(Function.getVars());
      pointerToHash = pointerToHash.changeReference(Function.getVars());
      
      int NUM_COMMITMENT_BITS_PER_BLOCK = 
          IntConstant.toIntConstant(Consts.fromName("NUM_COMMITMENT_BITS_PER_BLOCK")).toInt();
      //Data to store into
      BitStringStoreTarget bitsData = BitStringHelper.toBitStringStoreTarget(pointerToRecvData);
      int numBitsInMessage = bitsData.targetBits.size();
      
      if (NUM_COMMITMENT_BITS_PER_BLOCK%8 != 0 || numBitsInMessage%8 != 0){
        throw new RuntimeException("Sizes not a multiple of 8: Commitment bits = "+NUM_COMMITMENT_BITS_PER_BLOCK+", message length in bits = "+numBitsInMessage);
      }
      
      StructType committed_state_t = new StructType();
      committed_state_t.addField("secret", 
          new ArrayType(new RestrictedUnsignedIntType(8), NUM_COMMITMENT_BITS_PER_BLOCK/8));
      committed_state_t.addField("message", 
          new ArrayType(new RestrictedUnsignedIntType(8), numBitsInMessage/8));
      ...
      addStatement(new GenericGetStatement(GenericGetStatement.COMMITMENT_LOOKUP, committed_state.ptr, pointerToHash), sb);
      committed_state.data = committed_state.data.changeReference(Function.getVars());
      ...
      LvalExpression committed_data = LvalExpression.toLvalExpression(
          new SFE.Compiler.UnaryOpExpression(new StructAccessOperator("message"),committed_state.data));
      LvalExpression secret =  LvalExpression.toLvalExpression(
          new SFE.Compiler.UnaryOpExpression(new StructAccessOperator("secret"),committed_state.data));
      
      ArrayList<SFE.Compiler.Expression> args_to_commitment_hash = new ArrayList();
      //uint8_t* message, int num_bits_message, uint8_t* key, commitment_t* hash
      ...
      genericFunctionCall(scope, 
          "commitment_hash",
          parse_uid,
          sb,
          args_to_commitment_hash
          );
      computed_hash.data = computed_hash.data.changeReference(Function.getVars());

      //Committed hash
      LvalExpression commitment_hash = 
          LvalExpression.toLvalExpression(
              new SFE.Compiler.UnaryOpExpression(new PointerAccessOperator(), pointerToHash));
      //compare commitment_hash against computed_hash
      //and emit a bunch of "assertZero's"
      
      for(int i = 0; i < commitment_hash.size(); i++){
        addStatement(new AssertZeroStatement(
      ...
      //The first type is the type of commited_data, the second type is that of the store target.
      //raw bitstring is the intermediate
      BitString.toBitString(committed_data.getDeclaredType(), sb, committed_data).toAssignments(bitsData.targetCVar, sb);
  ...
  private void requireHashT() {
    if (!typetable.containsKey("hash_t")){
      throw new RuntimeException("Add #include<db.h> to use built-in hashing and commitment functions");
    }
  }
  private Type CTypeForUOE(SFE.Compiler.UnaryOpExpression uoe) {
    ...
    if (uoe.op instanceof PointerAccessOperator){
      ...
      if (!(middle instanceof StructType)){
        throw new RuntimeException("Could not "+uoe.op.toString()+" on "+uoe.getMiddle()+", which is not a struct.");
      }
  ...
  private void builtinFunctions() {
    ...
    addFunction(new FunctionDefinition(new Identifier(CBuiltinFunctions.RAMGET_NAME), null));
    addFunction(new FunctionDefinition(new Identifier(CBuiltinFunctions.RAMPUT_NAME), null));
  }
  ...
  private SFE.Compiler.Statement as(LvalExpression lval, SFE.Compiler.Expression rhs) {
    //return new SFE.Compiler.AssignmentStatement(lval, rhs);
    //Use protoAssignments because they can allow assignments to varying sizes when pointer indirection is involved
  ...
  /**
   * Type qualifiers such as static and const must be stripped off before calling this
   * function.
   */
  private Type getCTypeBase(TypeSpecification ts) {
    ...
    while(true){
      ...
      if (parts[0].equals("static")){
        System.out.println("WARNING: Compiler ignoring 'static' keyword in "+ts.spec);
        spec = parts[1];
      } else if (parts[0].equals("const")){
        System.out.println("WARNING: Compiler may allow multiple assignments to 'const' variable with type "+ts.spec);
        spec = parts[1];
      } else if (parts[0].equals("inline")){
        System.out.println("WARNING: Compiler ignoring 'inline' keyword in "+ts.spec);
        spec = parts[1];
      } else {
        break;
      }
   ...
   * Constant expressions can basically just be arithmetic expressions of integer constants
   * or const variables which have also been assigned constant expressions.
   */
  private int toInt(Expression k) {
    if (k instanceof Constant) {
      ...
      throw new RuntimeException("Constant handling of "+kop.operator+" op expressions not yet handled");
