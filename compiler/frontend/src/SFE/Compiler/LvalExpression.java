...
/*
  ...
  */
  ...
  public Expression evaluateExpression(String goal, String tempName, BlockStatement result) {
    //no reason to create a temporary, we are already an Lval
    return this;
  }
  ...
  public Expression inline(Object obj, StatementBuffer assignments) {
    if (true){
      return new UnaryOpExpression(new UnaryPlusOperator(), this).inline(null, assignments);
    }
    ...
    if (assigningStatement instanceof AssignmentStatement) {
      ...
      if (referenceCountUB == 1 &&
        ...
        /*
         Unsafe, references may already be established...
        for(Expression toRet : ((AssignmentStatement) as).getAllRHS()) {
    ...
    if (as != null){
      for(Expression q : as.getAllRHS()){
        ...
        if (asLval != null){
          ...
          if (cand != null){
            //Multiple bitStrings may exist for this lval, keep the shortest
