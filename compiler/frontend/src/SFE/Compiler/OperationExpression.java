...
public abstract class OperationExpression extends Expression
  ...
  /**
   * recursively calculates inner arithmetic expression to a scalar type in an lvalue
  ...
  public Expression evaluateExpression(String goal, String tempName, BlockStatement result) {
    ...
    if (op instanceof UnaryPlusOperator) {
      return ((UnaryOpExpression)this).getMiddle().evaluateExpression(goal, tempName, result);
    }
    
    LvalExpression tmpLvalExp = Function.addTempLocalVar(goal + Expression.TEMPORARY_SEPARATOR + tempName /*+ tempLabel*/, new BusType());
