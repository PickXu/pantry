...
public abstract class ScalarOperator extends Operator implements SLPTReduction{
  public Statement toSLPTCircuit(Object obj) {
    ...
    if (arity() == 2){
      ...
      throw new RuntimeException("I don't know how to SLPTReduce an operator with arity "+arity()+" "+this);
    }
    return result;
  }
  ...
  public Expression fieldEltAt(Expression expression, int i) {
    if (i != 0){
      throw new RuntimeException("Array index out of bounds exception index "+i+" fieldEltAt a scalar operator "+this);
    }
    return expression;
  }
