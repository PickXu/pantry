...
public class CStyleCast extends Operator implements SLPTReduction, OutputWriter {
  ...
  public CStyleCast(Type targetType) {
    if (!(targetType instanceof IntType) || targetType.size() > 1){
      throw new RuntimeException("Currently can only do C-style casts to int types");
    }
  ...
  public Expression fieldEltAt(Expression expression, int i) {
    if (i != 0){
      throw new RuntimeException("Array index out of bounds exception index "+i+" fieldEltAt casting "+ this);
    }
    return expression;
  }
  ...
  public Statement toSLPTCircuit(Object obj) {
    ...
    if (middle.metaType instanceof ArrayType){
      subAs = new AssignmentStatement(lhs.lvalFieldEltAt(0),new UnaryOpExpression(this, middle));
    } else {
      middle = middle.evaluateExpression(lhs.getName(),"M", result); //produces SLPT circuit
      subAs = new AssignmentStatement(lhs.lvalFieldEltAt(0),new UnaryOpExpression(this, middle));
    }
  ...
  public Expression inlineOp(StatementBuffer sb, Expression ... args) {
    ...
    if (targetType instanceof PointerType){
      ...
      if (in.getType() instanceof ArrayType){
        Pointer toRet = Pointer.toPointer(in);
        if (!TypeHeirarchy.equals(toRet.getType().getPointedToType(), ((PointerType) targetType).getPointedToType())){
          throw new RuntimeException("I don't know how to cast "+in+" of type "+in.getType()+" to pointer-to-"+((PointerType) targetType).getPointedToType());
        }
      ...
      throw new RuntimeException("I don't know how to cast "+in+" of type "+in.getType()+" to "+targetType); 
    } else {
