...
public class Pointer extends Expression implements Inlineable, OutputsToPolynomial{
  ...
  /**
   * This doesn't exactly work because a "heap" value will, through this function, 
   * also appear on the stack. This makes sense if you see the heap as part of the stack's address space,
   * so that the heap appears multiple times in memory.
   * (so every heap variable has an address on the stack, but some stack variables don't have an address
   * in the heap, and all variables have distinct stack addresses)
   */
  public static int getMemoryLocation(LvalExpression lval) {
    ...
    while(true){
      if(c.getType() instanceof ArrayType){
        ...
        c = new StructAccessOperator(((StructType)c.getType()).getFields().firstElement()).resolve(c);
      } else {
        break;
      }
    }
