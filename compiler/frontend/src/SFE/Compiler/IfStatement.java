...
public class IfStatement extends Statement implements Optimizable {
  ...
  /**
   * Muxes the three scopes possible in an if statement together, writing generated statements
   * to the statement buffer.
   * 
   * 1) The current scope (Function.getVars())
   * 2) A scope reflecting variables generated by the end of the then block
   * 3) A scope reflecting variables generated by the end of the else block
   * 
   * Name conflicts can occur in multiple ways, mux statements are inlined to prevent
   * unecessary condition variable multiplication.
   */
  public static void expandMuxStatements(Expression condition, Map<String, LvalExpression> thenScope, Map<String, LvalExpression> elseScope, StatementBuffer statements){ 
    //Get the scope holding the if statement
    VariableLUT beforeIf = Function.getVars();
    ...
    for(String lvalName : ConflictVariablesSorted.values()) {
      ...
      //toAssignmentStatements_NoChangeRef is not compatible with toSLPTCircuit.
      //We must build a result that is already SLPT.
      if (false){
        //This doesn' work.
        AssignmentStatement valueas = new AssignmentStatement(valueMostRecent, (mux).toBinaryOps());
        ...
        // Evaluating complex expressions is tricky when we can't use toSLPT.
        // Have to unroll by hand:
        // M*(A - B) + B
        LvalExpression negB = evaluateExpression_if(
            new BinaryOpExpression(new TimesOperator(), IntConstant.NEG_ONE, mux.getValueFalse()),
        ...
        LvalExpression AminB = evaluateExpression_if(
            ...
            valueMostRecent.getName(), "AminB", statements
            );
        LvalExpression MtAminB = evaluateExpression_if(
            new BinaryOpExpression(new TimesOperator(), mux.getCondition(), AminB),
            valueMostRecent.getName(), "MtAminB", statements
            );
        AssignmentStatement muxAssign = new AssignmentStatement(valueMostRecent, 
            new BinaryOpExpression(new PlusOperator(),MtAminB, mux.getValueFalse()));
  ...
  /**
   * Evaluates expression toEvaluate into a temporary variable (see syntax for evaluateExpression).
   * 
   * However, the statements needed to evaluate toEvaluate are immediately used with toAssignmentStatements_NoChangeRef(sb).
   * 
   * Finally, the true result is returned (by getting the newest name from the VariableLUT).
   */
  private static LvalExpression evaluateExpression_if(Expression toEvaluate, String goal, String tempName, StatementBuffer sb){
    BlockStatement result = new BlockStatement();
    //This cast will fail if the operation is a compile time operator. One of the many reasons why this method
    //is an internal to the ifStatement class - it's not general purpose.
    LvalExpression tempValue = (LvalExpression)toEvaluate.evaluateExpression(goal, tempName, result);
    //run result
    for(Statement as : result.getStatements()) {
      ((AssignmentStatement)as).toAssignmentStatements_NoChangeRef(sb);
    }
    //Now this should be safe - we can rereference the temporary (but not the inputs that created it)
   ...
   */
  private static LvalExpression newLvalOrExisting(Expression expr, String prefixForNewLval, StatementBuffer statements) {
    LvalExpression existing = Optimizer.getLvalFor(expr);

    if (existing == null) {
      BlockStatement result = new BlockStatement();
      LvalExpression newLval = (LvalExpression) expr.evaluateExpression(prefixForNewLval, "condProduct", result);
      if (result.getStatements().size() > 1){
        throw new RuntimeException("Cannot pass a complex (nested) expression to newLvalOrExisting");
      }
      ...
      return newLval; //This is safe, because we just created the variable on the current scope
    } else {
      return existing;
    }
  }
