...
// read a block from bs and store it in another block store named by the block's hash.
void export_exo_inputs_from_bs(HashBlockStore *bs, void *ptr, int size, hash_t *digest) {
  __hashget(bs, ptr, digest, size);
  export_exo_inputs(ptr, size, digest); 
}
